...
detector = cv2.CascadeClassifier("F:/LockUnlock/lock/haarcascade_frontalface_default.xml")
...
faces,ids = getImagesAndLabels('F:/LockUnlock/dataset')  # Get the faces and IDs

recognizer.train(faces, np.array(ids)) # Train the model using the faces and IDs

assure_path_exists('F:/LockUnlock/lock/trainer/')    # Save the model into trainer.yml
recognizer.save('F:/LockUnlock/lock/trainer/trainer.yml')
